"use client";

import IncomeForm, { incomeFormSchema } from "@/components/income-form";
import { Category } from "@/types/Category";
import { format } from "date-fns";
import { useRouter } from "next/navigation";
import { toast } from "sonner";
import { z } from "zod";
import { createIncomeTransaction } from "./actions";

type Wallet = {
  id: number;
  name: string;
  currency: string;
};

export default function IncomeTransactionForm({
  categories,
  wallets,
}: {
  categories: Category[];
  wallets: Wallet[];
}) {
  const router = useRouter();

  const handleSubmit = async (data: z.infer<typeof incomeFormSchema>) => {
    const result = await createIncomeTransaction({
      amount: data.amount,
      transactionDate: format(data.transactionDate, "yyyy-MM-dd"),
      categoryId: data.categoryId,
      walletId: data.walletId,
      description: data.note || `Income transaction - ${new Date().toLocaleDateString()}`,
    });

    if (result.error) {
      toast.error(result.message);
      return;
    }

    toast.success("Income transaction created");
    router.push(
      `/dashboard/transactions?month=${data.transactionDate.getMonth() + 1}&year=${data.transactionDate.getFullYear()}`,
    );
  };

  const handleSaveAndAddMore = async (
    data: z.infer<typeof incomeFormSchema>,
  ) => {
    const result = await createIncomeTransaction({
      amount: data.amount,
      transactionDate: format(data.transactionDate, "yyyy-MM-dd"),
      categoryId: data.categoryId,
      walletId: data.walletId,
      description: data.note || `Income transaction - ${new Date().toLocaleDateString()}`,
    });

    if (result.error) {
      toast.error(result.message);
      return;
    }

    toast.success("Income transaction created");
    // Reset form by reloading the page
    window.location.reload();
  };

  return (
    <IncomeForm
      categories={categories}
      wallets={wallets}
      onSubmit={handleSubmit}
      onSaveAndAddMore={handleSaveAndAddMore}
    />
  );
}

